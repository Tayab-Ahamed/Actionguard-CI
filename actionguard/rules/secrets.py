...
def scan(repo_path: Path):
    ...
    for p in iter_files(repo_path):
        if p.name.startswith('actionguard-report'): continue
        text=read_text(p)
        if text is None: continue
        for label,rx in PATTERNS:
            for m in re.finditer(rx,text):
                line=text.count('\n',0,m.start())+1; preview=redact(m.group(0))
                out.append(Finding('AG-SEC-001','actionguard','secrets',Severity.CRITICAL,f'{label} detected',rel(p,repo_path),line,f'Redacted match: {preview}','A usable credential may be exposed to anyone with repository access.','Revoke or rotate the credential immediately, remove it from history, and use GitHub encrypted secrets.',False,False,manual_review_required=True))
    return out
