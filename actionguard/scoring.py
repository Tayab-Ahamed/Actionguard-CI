...
def calculate_scores(findings):
    def score(items):return max(0,100-sum(WEIGHTS[f.severity.value] for f in items))
...
def severity_counts(findings):
    c=Counter(f.severity.value for f in findings); return {x:c.get(x,0) for x in WEIGHTS}
